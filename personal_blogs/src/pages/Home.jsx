...
const Home = () => {
  const latestPosts = getAllPosts().slice(0, 5)  // Get exactly 5 articles
        ...
        <div className="articles__latest">
          {latestPosts.map(post => (
            <div key={post.id}>
              ...
              <p className="articles__article-date">{post.date}</p>
            </div>
          ))}
        </div>

        <p className="articles__text">
          <Link to="/blog" className="articles__link">View all articles</Link>
        </p>
      </section>
    </>
  )
